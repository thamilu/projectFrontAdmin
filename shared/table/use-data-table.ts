...
export interface TableState<TFilters = Record<string, unknown>> {
  page: number;
  size: number;
  sortBy?: string;
  sortDir?: 'asc' | 'desc';
...
export interface UseDataTableOptions<TFilters = Record<string, unknown>> {
  initialPage?: number;
  initialSize?: number;
...
export function useDataTable<TFilters = Record<string, unknown>>(options: UseDataTableOptions<TFilters> = {}) {
