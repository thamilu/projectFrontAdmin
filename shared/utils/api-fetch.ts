...
import { clientApiUrl } from '@/constants/api/client-url';
 ...
 * @example
 * ```ts
 * const data = await apiFetch<Product[]>('/v1/products');
 * ```
 */
...
export function transformBackendArray<T, R>(
  ...
  const rawItems = response && typeof response === 'object' && 'data' in response
  ...
  return (rawItems as T[]).map(mapper);
}
