...
const navItems: NavItem[] = [
  { title: 'Dashboard',       href: '/admin/dashboard',       icon: LayoutDashboard, requireAdmin: true },
...
function resolveIsAdmin(session: ReturnType<typeof useSession>['data']): boolean {
  ...
  const topRoles: string[] = (session as any)?.roles ?? [];
...
export function AdminSidebar() {
   ...
   *
   * This ref persists across renders without causing re-renders itself.
   */
  const confirmedAdminRef = useRef(false);
  useEffect(() => {
    if (isAdmin) confirmedAdminRef.current = true;
  }, [isAdmin]);

  // After session loads: show all items if admin (sticky), else filter
  const effectiveIsAdmin = isAdmin || confirmedAdminRef.current;
